...
class Search extends React.Component {
    ...
    fetchSearchResults = async isQueryInCache => {
        let urlSearch = `https://api.themoviedb.org/3/search/multi?api_key=4cc894b01eadfbbfbff1135572164973&query=${this.state.query}&page=${this.state.page}`;
        let data;
            ...
            <Hom>
                {this.state.query && this.state.areResultsFound && (
                    <section className="search-results">
                        <Helmet>
                            <title>{`Resultados da busca por "${this.props.query}" | DWFilmes`}</title>
                        </Helmet>
                        <h1 className="heading search-main-head result-heading home-heading color-orange">
                            {`Resultados da busca por "${this.props.query}"`}
                        </h1>
                ...
                {!this.state.query && (
                    <>
                        ...
                        <LoadingAnimation
                            message="Pesquise qualquer filme, tv ou celebridade"
                            animation={false}
                        />
                    </>
                )}
                ...
                {this.state.query && !this.state.areResultsFound && (
                    <NotFound
                        message={`Nenhum resultado de pesquisa encontrado para ${this.state.query}`}
                    />
                )}
            </Hom>
        );
